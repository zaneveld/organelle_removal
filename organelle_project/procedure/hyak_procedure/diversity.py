...
for study in studies:
    ...
    for denoiser in denoisers:
        phylogeny_path = glob(join(working_dir, 'output', (study + '_' + denoiser + '_rooted_tree.qza')))[0]
        ...
        for classifier in classifiers:
            ...
            filtered_base_table.save(join(working_dir, 'output', 'diversity', (study + '_' + denoiser + '_silva_' + classifier + '_harmonized_filtered_table.qza')))
                ...
                with tempfile.TemporaryDirectory() as temp_dir:
                    for column in columns_of_interest[study]:
                        ...
                        for a_metric in alpha_diversity_metrics:
                            qzv = Visualization.load(join(working_dir, 'output', 'diversity', (study + '_' + denoiser + '_' + reference + '_' + classifier + '_' + a_metric + '.qzv')))
                        ...
                        for b_metric in beta_diversity_metrics:
                            qzv = Visualization.load(join(working_dir, 'output', 'diversity', (study + '_' + denoiser + '_' + reference + '_' + classifier + '_' + column + '_' + b_metric + '.qzv')))
                            qzv.export_data(temp_dir)
