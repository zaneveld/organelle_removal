...
lanes = {'human_gut':14}#, 'milk':6, 'peru_ants':1, 'song':13, 'mocks':10, 'GSMP':2, 'GCMP':2}
...
#    deblur_merged_seqs.save(join(working_dir, 'input', study + '_deblur_filtered_merged_seqs.qza'))
 ...
 #   deblur_unfiltered_merged_table, = merge(deblur_unfiltered_tables, 'sum')
  #  deblur_unfiltered_merged_table.save(join(working_dir, 'output', study + '_deblur_unfiltered_merged_ft.qza'))
   # deblur_unfiltered_merged_seqs, = merge_seqs(deblur_unfiltered_seqs)
    #deblur_unfiltered_merged_seqs.save(join(working_dir, 'output', study + '_deblur_unfiltered_merged_seqs.qza'))

    d2_merged_seqs = Artifact.load(join(working_dir, 'input', study + '_dada2_unfiltered_merged_seqs.qza'))
    d2_merged_table = Artifact.load(join(working_dir, 'input', study + '_dada2_unfiltered_merged_ft.qza'))
    ...
    d2_filtered_merged_table.save(join(working_dir, 'input', study, study + '_' + '_dada2_filtered_merged_ft.qza'))
    d2_filtered_merged_seqs.save(join(working_dir, 'input', study, study + '_' + '_dada2_filtered_merged_seqs.qza'))
